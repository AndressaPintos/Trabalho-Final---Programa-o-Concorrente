...
typedef struct {
    const byte *hashSenha; //recebe o hash original
   // int TamSenhaAtual;            // tamanho da senha corrente
    int inicioVet;      // índice inicial no array letras para a posição 0
    int fimVet;        // índice final
    atomic_int *encontrado;  // flag atômica compartilhada entre threads
} arg_t;
...
*/
void iterate_recursive(const byte *hashSenha, char *str, int posAtual, int TamSenhaAtual, atomic_int *encontrado) {
    int i;
    int qtdeLetras = (int) strlen(letras);

    if (atomic_load(encontrado)) 
        return; // já achou, retorna rápido

    if (posAtual < (TamSenhaAtual - 1)) {
        for (i = 0; i < qtdeLetras && !atomic_load(encontrado); i++) {
            str[posAtual] = letras[i];
            iterate_recursive(hashSenha, str, posAtual + 1, TamSenhaAtual, encontrado);
        }
    } else {
        for (i = 0; i < qtdeLetras && !atomic_load(encontrado); i++) {
            str[posAtual] = letras[i];
...
void *buscarSenha(void *argp) {
    ...
    for (int tam=1; tam <= MAX && !atomic_load(arg->encontrado); ++tam){
        local_str[tam] = '\0';//adiciona o \0 para indicar o fim da string
        for (int i = arg->inicioVet; i < arg->fimVet && !atomic_load(arg->encontrado); i++) {
            ...
            if (tam == 1) {
                ...
                if (memcmp(arg->hashSenha, hash2, MD5_DIGEST_LENGTH) == 0) {
                    local_str[1] = '\0';
                    printf("Senha encontrada: %s\n", local_str);
                    atomic_store(arg->encontrado, 1);
                    break;
                }
            } else {
                // completar recursivamente a partir da posição 2
                iterate_recursive(arg->hashSenha, local_str, 1, tam, arg->encontrado);
            }
        }
        if (atomic_load(arg->encontrado))
             break;
    }
...
int main(int argc, char **argv) {
    ...
    byte hashSenha[MD5_DIGEST_LENGTH];
    ...
    atomic_int encontrado;
    atomic_init(&encontrado, 0);
    int qtdeLetras = (int) strlen(letras);
